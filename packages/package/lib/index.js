...
const { formatPath, isObject } = require("./utils");
...
class Package {
  ...
  async prepare() {
    if (this.targetPath && !pathExists(this.targetPath)) {
      ...
      fsExtra.mkdirpSync(this.targetPath);
    }
    ...
    if (this.pkgVersion === "latest") {
      this.pkgVersion = await getNpmLatestVersionNum(this.pkgName);
    }
  }
  ...
  async exists() {
    ...
    if (this.storePath) {
      ...
      await this.prepare();
      log.verbose("exists", await pathExists(this.cacheFilePath));
  ...
  async update() {
    ...
    // 1、查最新的版本号
    // 2、版本路径是否存在于本地
    const lastestVersion = await getNpmLatestVersionNum(this.pkgName);
