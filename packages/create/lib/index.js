...
const { spinnerStart, sleep, execAsync } = require("@keroro-cli/utils");
...
class CreateCommand extends Command {
  ...
  constructor(argv) {
    ...
    log.verbose("localPath", process.cwd());
  ...
  async execute() {
    try {
      ...
      log.error("create command - execute", e.message);
    }
  }
  ...
  createTemplateChoice() {
    return this.projectTemplates.map(({ npmName, name }) => ({
      name,
      value: npmName,
    }));
  }
  ...
  async _downloadTemplate() {
    ...
    // 在缓存目录下创建template文件夹
    const targetPath = path.resolve(userHome, ".keroro-cli-cli", "template");
